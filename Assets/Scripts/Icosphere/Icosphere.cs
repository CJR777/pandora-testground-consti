// courtesy of Peter Winslow: https://github.com/Pouchmouse/Procedural-Planet-Tutorial/tree/Tutorial1-IcosahedronGeneration (modified)
...
public class Icosphere
{
    ...
    public Icosphere (int resolution, Vector3 origin, float size)
    {
        Init();
        Subdivide(resolution);
        ScaleAndPosition(origin, size);
    ...
    public Mesh ReturnMesh()
    {
        ...
        {
            float pX = (verts[i].x * perlinScale) + (Time.timeSinceLevelLoad * waveSpeed);
            float pZ = (verts[i].z * perlinScale) + (Time.timeSinceLevelLoad * waveSpeed);
    ...
    private void ScaleAndPosition(Vector3 position, float scale)
    {
        for (int i = 0; i < m_Vertices.Count; i++)
        {
            Vector3 newVec = m_Vertices[i] * scale;
            newVec += position;
            m_Vertices[i] = newVec;
        }
    }
